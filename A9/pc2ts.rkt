...
(define pc2ts
  (lambda (file-name source-name)
    ;; WARNING: pc2ts will erase existing files when generating new ones!
      ...
      (let ([src (open-output-file source-name)])
        (dynamic-wind
         ...
         (lambda ()
           ...
           (display (pc2ts-header decl*) src)
           (check-correct-info)
           ;; write a generated source file source-name
           (display (pc2ts-source) src)
           (set! global-decls '())
           (set! union-defs ""))
         (lambda ()
    ...
    (apply string-append
           (map (lambda (elt)
                  (cond
                    ...
                    [(string? elt) elt]
                    [else (error 'pc2ts-append "Invalid argument ~s" elt)]))
                args))))

(define pc2ts-gen-unions
  (lambda (union)
                    ...
                    (let* ([safe-tag (safe tag)]
                           [fnname (cond
                                     [(memv tag js-keywords)
                                      => (λ (p) (pc2ts-append name safe-tag))]
                 ...
                 [(eq? (car last) 'else)
                  (let* ((test0 ((parse-function-body #f env level) (caar c*)))
                    ...
                    (pc2ts-append
                     ...
                     (apply string-append
                            (map (lambda (x y)
                                   (pc2ts-append (tabs level) "} else if (" x ") {\n"
                                                 y))
                                 test* body*))
                     (tabs level) "} else {\n" body "    }\n"))]
                 [else
            ...
            [`,x #:when (symbol? x) (letrec ([var (apply-env env x)]
                                      ...
                                      (if (is-global? var)
                                          (begin
                                            ...
                                            (global var))
                                          safe-x))]
            [`,x #:when (integer? x) (pc2ts-append x)]
            [`(zero? ,x) 
...
(define pc2ts-source
  (lambda ()
    (let* ([s1 (apply string-append (map pc2ts-gen-unions reg-unions))]
